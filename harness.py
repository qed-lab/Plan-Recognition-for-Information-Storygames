...
def run_planner(domain, problem, output_file='execution.details', trace_file=os.devnull, timeout_seconds=PLAN_TIME_LIMIT_MIN,
    ...
    if timeout_seconds is not None:
        ...
        tl += "--overall-time-limit {}".format(int(ceil(timeout_seconds)))
    else:
        timeout_seconds = 100
        tl += "--overall-time-limit 100"
    ##
    
    # sasf = domain.split("/")[1] + ".sas"
    
    cmd = "python3 fast-downward/fast-downward.py {} --plan-file {} {} {} --search \"astar(lmcut())\"".format(tl, output_file, domain, problem)
    
    # if bound is not None:
        # cmd += " --bound {} ".format(bound)
    print(cmd)
    start_time = TIMER.time()
    # ret = os.system(cmd + DEVNULL)
    ret = None
    try:
        with open('outfile', "w") as out:
            result = subprocess.run(cmd, stdout=out, shell=True)
        ##
    ...
    if ret == 0:
        obscure_blind.read_std_output_fd('outfile', output_file)
        plan, cost, _ = obscure_blind.read_fd_plan_details(output_file)
        #                                           domain_file, problem_file, sln_file, output_file
        if trace_file == os.devnull:
            print("Trace not being generated")
        else:
            print(f"Trace going to {trace_file} ...")
            fd_trace_maker.generate_trace_from_solution(domain, problem, output_file, trace_file)
        ##
...
def read_hypotheses_and_get_costs(basename, domain_f, hyps_f, template_f):
    ...
    try:
        os.makedirs(basename + "_fd-hypotheses")
    except FileExistsError as e:
        pass
        # print("unable to make folder ", basename + "_fd-hypotheses")
        # print("\t", e)
    ##
    
    # Compute optimal unconstrained solutions for each hypothesis:
    for hyp in range(len(hyps)):
        hyp_problem = basename + "_fd-hypotheses/hyp_{}_problem.pddl".format(hyp)
        fill_template_pddl(template_f, hyp_problem, hyps[hyp])
        hyp_sol = basename + "_fd-hypotheses/hyp_{}.solution".format(hyp)
        hyp_trace = basename + "_fd-hypotheses/hyp_{}.trace".format(hyp)
        if not os.path.exists(hyp_sol):
            _, cost, time = run_planner(domain_f, hyp_problem, hyp_sol, hyp_trace, 1000)
        else:
            _, cost, time = obscure_blind.read_fd_plan_details(hyp_sol)
        if cost is None:
            os.system(f"rm -rf {hyp_sol} {hyp_trace} {hyp_problem}")
            continue
...
def write_observations_settings(settings, folder, problemname, overwrite=False, max_num_tot_orders=25):
    ...
    domain_f = folder + "/domain.pddl" # basename + "domain.pddl"
...
def write_observations_setting(folder, problemname, sett, hyp_solutions, hyp_traces, overwrite=False, max_num_tot_orders=25):
    ...
    for true_hyp in range(len(hyp_solutions)):
        ...
        optimal_steps, _, _ = obscure_blind.read_fd_plan_details(hyp_solutions[true_hyp])
        ...
        if os.path.exists(obs_f):
            ...
            if obs_count == 0:  # No observations means everything is indicated
                ...
                for hyp in range(len(hyps)):
                    ...
                    # Comment this out if you want to review the solutions
                    os.system("rm -rf {}".format(obs_hyp_sol))
...
def run_domain(domain, settings, problemnames=None, time=PLAN_TIME_LIMIT_MIN):
    ...
    evaluate_domain(folder, settings,problemnames, result_file=folder + "/fd-results.object", time_limit=time)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Evaluate a domain, or process the results from a run")
    # parser.add_argument('domain', default='block-words', nargs='?', help="Choices: ['block-words', 'easy-grid-navigation', 'easy-ipc-grid', 'logistics', 'campus', 'depots'] ")
    ...
    if args.process is not None:
        result_file = "Benchmark_Problems/" + args.domain + "/fd-results.object" if args.process == "USE_DOMAIN" else args.process
        ...
        for domain in ["kitchen", "campus", "depots", "driverlog", "dwr", "block-words", "easy-ipc-grid", "ferry", "intrusion-detection", "kitchen", "logistics", "miconic", "rovers", "satellite", "sokoban", "zeno-travel"]:
            write_observation_domain_settings(settings, "Benchmark_Problems/" + domain, max_num_tot_orders=args.max_ordered)
            print("Running domain '{}', which will take {} runs. Results will be regularly updated to a 'fd-results.object' in the domain's directory.".format(domain,
                                                                                                                                             count_runs_domain("Benchmark_Problems/"+domain, settings, args.problems, max_ordered=args.max_ordered)))
            # settings, folder, problemnames=None, overwrite=False, max_num_tot_orders=25
            try:
                run_domain(domain, settings, args.problems, args.time)
            except Exception as e:
                with open("err.txt", "a") as f:
                    print(">>> ERRROR {} <<<<\n".format(domain), file=f)
                    print(e, file=f)
                    print("\n>>> ERRROR {} <<<<".format(domain), file=f)
                    print("\n\n\n", file=f)
                ##
            ##
        ##
