...
def read_fd_plan_details(solution_file_name):
    steps = []
    cost = None
    total_time = None
    ...
    with open(solution_file_name, 'r') as file:
        for line_count, line in enumerate(file):
            ...
            elif "step count" in line:
                pass
            else:
                steps.append(action_observation(line.replace("(", "").replace(")", "").split()))
            ##
        ##
    ##
...
def read_std_output_fd(outfile, sln_file):
    step_count = None
    total_time = None
    ...
    with open(outfile, 'r') as file:
        for line_count, line in enumerate(file):
            if "Search time:" in line:
                total_time = float(line.split(" ")[5][:-2])
            ##
            if "Plan length:" in line:
                step_count = int(line.split(" ")[5])
            ##
        ##
    ##
    
    with open(sln_file, "a") as file:
...
def obscure_AF(steps, states, observed_perc, unordered_perc, garble_perc, fluent_visibility):
    ...
    garbled_indices = set()
    attempts = 0
    while len(garbled_indices) < garbled_amount and attempts < 50:
        rand_idx = random.randint(0, len(observations) - 1)
        if isinstance(observations[rand_idx], action_observation):
            if len(observations[rand_idx].action) > 1 and rand_idx not in garbled_indices:
        ...
        #print(">>> if you see me forever, oops <<<")
        #print(garbled_amount, "  :  ", len(garbled_indices), "//////", num_actions)
    ##
